...
# UPDATED: Now uses the new API endpoint with enhanced error handling
...
IMAGE_TIMEOUT = 10 # Increased timeout
CANVAS_SIZE = (800, 800)
...
def fetch_and_process_image(image_url: str, size: tuple = None):
    try:
        ...
        return None
    except Exception as e: # Catches PIL errors, etc.
...
def outfit_image():
    ...
    # ... (rest of the code for image composition remains the same) ...
    bg_w, bg_h = background_image.size
    if CANVAS_SIZE is None:
        canvas_w, canvas_h = bg_w, bg_h
        scale_x = scale_y = 1.0
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=PORT, debug=True)    try:
        resp = session.get(image_url, timeout=IMAGE_TIMEOUT)
        resp.raise_for_status()
        img = Image.open(BytesIO(resp.content)).convert("RGBA")
        if size:
